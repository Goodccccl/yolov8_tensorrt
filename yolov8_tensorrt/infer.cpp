...
std::vector<Detection> Arrange_outputs(std::vector<float*> outputs, int output_anchorsNb, int output_anchorsOne, float set_conf)
{
	...
	for (int i = 0; i < output_anchorsNb; i++)
	{
		...
		{
			temporary.box[0] = anchorOne[0];
			temporary.box[1] = anchorOne[1];
			temporary.box[2] = anchorOne[2];
			temporary.box[3] = anchorOne[3];
...
std::vector<Detection> infer(Parameters param, const std::string enginePath, const std::string imgPath)
{
	...
	int outputs_num = 1;
	//int new_w, new_h;
	//float* src = preprocess(imgPath, param.input_height, param.input_width, new_w, new_h);
	float* src = preprocess(imgPath, param.input_height, param.input_width);
